...
# /_/   /___/_/-Bash
#
# Fuzzy Finder Bash integration.
#
# Shortcuts:
#   CTRL-F - Paste the selected file path into the command line.
#   CTRL-R - Paste the selected command from history into the command line.
#   CTRL-J - (jump) cd into the selected directory.
#
# Completion:
#   $FZF_TMUX               (default: 0)
...
#
# Alias:
#     fzf-keys - Print cheatsheet.


# If fzf not exist, return
type fzf &> /dev/null || return

# Shortcuts help
alias fzf-keys="head -n 21 $BASHRC_MODULES/autoload/fzf.sh"
...
if [[ ! -o vi ]]; then
  ...
  # CTRL-J - (jump) cd into the selected directory
  bind '"\C-j": " \C-e\C-u`__fzf_cd__`\e\C-e\er\C-m"'
else
  
  # CTRL-E - Shell expand line in $EDITOR
  ...
  bind '"\C-j": "\C-x\C-addi`__fzf_cd__`\C-x\C-e\C-x\C-r\C-m"'
